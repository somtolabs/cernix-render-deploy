...
@section('content')
<style>
    /* Restored original CERNIX homepage direction */
    .home-frame { min-height: 100vh; background: var(--bg); }
    .hero {
    ...
    .logo-mark img { filter: drop-shadow(0 10px 20px rgba(15,32,80,.08)); }
    ...
    .portal:nth-child(4) { animation: fadeUp .4s .42s ease both; }
    .portal:hover, .portal:focus-visible {
        transform: translateY(-3px);
        box-shadow: 0 12px 32px -8px rgba(15,32,80,.14), 0 4px 10px -4px rgba(15,32,80,.08);
    ...
    @media (min-width: 920px) {
        ...
        .hero { border-radius: 28px; padding: 70px 34px 52px; }
        .portals { padding: 0; }
        .system-status { grid-column: 1 / -1; width: min(600px, 100%); margin-bottom: 20px; }
        .footer-meta { grid-column: 1 / -1; padding-bottom: 28px; }
    }
    @media (max-width: 520px) {
        .hero { padding: 56px 20px 38px; }
        .stat-strip { border-radius: 16px; }
        .portal { padding: 18px; border-radius: 18px; }
        .portal .ico { width: 50px; height: 50px; }
    }
...
<div class="home-frame">
    ...
    <div class="portals">
        <a href="{{ route('student.register') }}" class="portal">
        ...
        <a href="{{ route('admin.login') }}" class="portal">
            <span class="accent-line"></span>
            <div class="ico admin">
                <svg width="24" height="24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
