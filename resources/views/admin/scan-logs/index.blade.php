...
<style>
    @media (max-width:700px) {
        .scan-log-table { overflow:visible; border:0; background:transparent; }
        .scan-log-table table, .scan-log-table tbody, .scan-log-table tr, .scan-log-table td { display:block; width:100%; }
        .scan-log-table thead { display:none; }
        .scan-log-table tr { margin-bottom:10px; padding:12px; border:1px solid var(--line); border-radius:16px; background:rgba(244,247,252,.72); word-break:normal; writing-mode:horizontal-tb; }
        .scan-log-table td { border:0; padding:7px 0; display:grid; grid-template-columns:minmax(88px,.38fr) minmax(0,1fr); gap:12px; align-items:start; }
        .scan-log-table td::before { content:attr(data-label); color:var(--ink-3); font-size:10px; font-weight:900; text-transform:uppercase; letter-spacing:.08em; }
        .scan-log-table td > * { min-width:0; justify-self:start; }
    }
</style>
...
<section class="admin-section"><div class="admin-section-head"><h2>Scan Records</h2><span>{{ $logs->total() }} records</span></div><div class="admin-section-body">
    ...
    <div class="admin-table-wrap scan-log-table"><table class="admin-table"><thead><tr><th>Time</th><th>Decision</th><th>Student</th><th>Matric</th><th>Examiner</th><th>Review Status</th><th>Action</th></tr></thead><tbody>
        @forelse($logs as $log)
            <tr id="scan-{{ $log->log_id }}"><td class="mono" data-label="Time">{{ $log->timestamp }}</td><td data-label="Decision"><span class="admin-status {{ $log->decision === 'APPROVED' ? 'green' : ($log->decision === 'DUPLICATE' ? 'amber' : 'red') }}">{{ $log->decision === 'DUPLICATE' ? 'REPEATED' : $log->decision }}</span></td><td data-label="Student">{{ $log->student_name ?? 'Student unavailable' }}</td><td class="mono" data-label="Matric">{{ $log->matric_no ?? 'Not available' }}</td><td data-label="Examiner">{{ $log->examiner_name ?? $log->examiner_username ?? 'Not available' }}</td><td data-label="Review">{{ $log->decision === 'DUPLICATE' ? 'Needs review' : 'Recorded' }}</td><td data-label="Action"><a class="admin-action ghost" href="{{ route('admin.scan-logs.show', $log->log_id) }}">View</a></td></tr>
        @empty
